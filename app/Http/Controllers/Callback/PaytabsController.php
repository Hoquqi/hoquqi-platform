...
use App\Models\ProjectBidWebhook;
use App\Models\ProjectBid;
use App\Models\ProjectBidTransaction;

use App\Models\ProjectBidUpgrade;
use App\Models\ProjectSubscription;
use App\Models\ProjectSubscriptionTransaction;
use App\Models\Project;
use App\Models\ProjectSubscriptionWebhook;
...
class PaytabsController extends Controller
{
        ...
        try {
            ...
            if ( $transaction_id ) {
                ...
                if ( is_array($response) && $response['success'] === TRUE) {
                    ...
                    // Get order id
                    $order_id = $request->get('cartId');


                    // Check if project subscriptions callback
                    if (Str::startsWith($order_id, "BB")) {
                        
                        // Get saved webhook data in our database
                        $data = ProjectBidWebhook::where('payment_id', $order_id)
                                                ->where('payment_method', $this->gateway)
                                                ->where('status', 'pending')
                                                ->firstOrFail();
        
                        // Handle deposit callback
                        $this->projectBid($data->user_id,$data->bid_id, $data->amount, $order_id, $data->subscription_id);
                    ...
                    // Check if project subscriptions callback
                    if (Str::startsWith($order_id, "SS")) {
                        
                        // Get saved webhook data in our database
                        $data = ProjectSubscriptionWebhook::where('payment_id', $order_id)
                                                ->where('payment_method', $this->gateway)
                                                ->where('status', 'pending')
                                                ->firstOrFail();
        
                        // Handle deposit callback
                        $this->projectSubscription($data->user_id,$data->project_id, $data->amount, $order_id);
                        ...
                        $subscription = ProjectSubscription::find($data->subscription_id);
                        // Update subscription
        ...
        switch ($type) {
            ...
            case 'projects':
                ...
                if ($status === 'success') {
                    ...
                    // Redirect to deposit history page
                    return redirect('account/projects')->with('success', __('messages.t_ur_transaction_has_completed'));
            ...
            case 'bids':
                ...
                if ($status === 'success') {
                    ...
                    // Redirect to deposit history page
                    return redirect('seller/projects/bids')->with('success', __('messages.t_ur_transaction_has_completed'));
     ...
     * @return void
     */
    private function projectSubscription($user_id,$project_id, $amount, $payment_id)
    {
        try {
            ...
            // Make transaction
            $deposit                 = new ProjectSubscriptionTransaction();
            $deposit->user_id        = $user_id;
            ...
            Project::find($project_id)->update(['status'=>'active']);
            
            // ProjectSubscription::where('project_id',$project_id)->update(['status'=>'active']);
     ...
     * @return void
     */
    private function projectBid($user_id,$bid_id, $amount, $payment_id, $subscription_id)
    {
        try {
            ...
            ProjectBid::find($bid_id)->update(['status'=>$settings->auto_approve_bids ? 'active' : 'pending_approval']);

            // Make transaction
            $deposit                 = new ProjectBidTransaction();
            $deposit->user_id        = $user_id;
            $deposit->bid_id        = $bid_id;
            $deposit->transaction_id = $payment_id;
            $deposit->payment_method = $this->gateway;
            $deposit->amount_total   = $amount;
            $deposit->amount_fee     = $fee;
            ...
            $subscription = ProjectBidUpgrade::find($subscription_id);
            $subscription->payment_method = $this->gateway;
            $subscription->payment_id     = $deposit->transaction_id;
            $subscription->status         = 'paid';
            $subscription->save();
