...
class OfflineComponent extends Component
{
    ...
    public function mount()
    {
        ...
        // Get settings
        $settings     = payment_gateway($payment_slug, false, true);
        
        // Check if has settings
        if ($settings && $settings->percentage_fee && $settings->fixed_fee) {
    ...
    public function update() 
    {
        try {
            ...
            // Get old settings
            $settings = payment_gateway($this->slug, false, true);
            ...
            // Update payment gateway
            OfflinePaymentGateway::where('slug', $this->slug)->update([
                'name'               => $this->name,
                'logo_id'            => $logo_id,
            ...
            // Update cache
            payment_gateway($this->slug, true, true);
