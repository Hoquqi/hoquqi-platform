...
class PaypalComponent extends Component
{
    ...
    // Api request link
    public $sandbox_link    = "https://www.sandbox.paypal.com/cgi-bin/webscr/";
    public $production_link = "https://www.paypal.com/cgi-bin/webscr/";
    ...
    public function mount()
    {
        ...
        if ($settings && $settings->percentage_fee && $settings->settings && $settings->fixed_fee) {
            ...
            $this->fill([
                ...
                'client_secret' => isset($api_settings['client_secret']) ? $api_settings['client_secret'] : null,
    ...
    public function update() 
    {
        try {
            ...
            if ($this->env === "sandbox") {
                ...
                \Log::info('[Config] paypal.mode : ' . 'sandbox');
                \Log::info('[Config] paypal.sandbox.client_id : ' . $this->client_id);
                \Log::info('[Config] paypal.sandbox.client_secret : ' . $this->client_secret);

            } else {

                // Write live config
                \Log::info('[Config] paypal.mode : ' . 'live');
                \Log::info('[Config] paypal.live.client_id : ' . $this->client_id);
                \Log::info('[Config] paypal.live.client_secret : ' . $this->client_secret);

            }

            // Update currency
            \Log::info('[Config] paypal.currency : ' . $this->currency);
