...
class SearchComponent extends Component
{
    ...
    public function getGigsProperty()
    {
        ...
        // start a new query
        $query = Gig::query()->active()->orderByDesc('id');
        ...
        if ($this->sort_by) {
            switch ($this->sort_by) {
                ...
                case 'popular':
                    ...
                    $query = Gig::query()->active()->orderByDesc('counter_visits');
                    break;

                // Best rating
                case 'rating':
                    //$query->orderByDesc('rating');
                    $query = Gig::query()->active()->orderByDesc('rating');
                    break;

                // Most sales
                case 'sales':
                    //$query->orderByDesc('counter_sales');
                    $query = Gig::query()->active()->orderByDesc('counter_sales');
                    break;
                ...
                case 'price_low_high':
                    ...
                    // $query = Gig::query()->active()->orderBy('price', 'ASC');
                    $query = Gig::query()->active()->orderByRaw('CONVERT(price, SIGNED) asc');
                    break;

                // Price high to low
                case 'price_high_low':
                    //$query->orderBy('price', 'DESC');
                    // $query = Gig::query()->active()->orderBy('price', 'DESC');
                    $query = Gig::query()->active()->orderByRaw('CONVERT(price, SIGNED) desc');
                    break;
                
                default:
                    //$query->orderByRaw('RAND()');
                    $query = Gig::query()->active()->orderByRaw('RAND()');
                    break;
            }
        }
