...
class CheckoutComponent extends Component
{
    ...
    public function confirm($data = null)
    {
        try {
            ...
            if ($this->selected_method === 'wallet') {
                ...
                if ($this->calculateExchangeAmount() > $available_balance) {
            ...
            if ($selected) {
                ...
                switch ($this->selected_method) {
                    ...
                    case 'stripe':
                        ...
                        // $this->notification([
                        //     'title'       => __('messages.t_error'),
                        //     'description' => json_encode($intent),
                        //     'icon'        => 'error'
                        // ]);
                        ...
                        // Successfull payment
                        // $this->success('stripe', $intent->client_secret);
                    ...
                    case 'paytabs':
                        ...
                        // Get total amount
                        $amount              = $this->calculateExchangeAmount($gateway_currency_exchange);
    ...
    protected function webhook($data)
    {
        try {
            ...
            // Get payment gateway exchange rate
            $gateway_currency_exchange = (float)settings($payment_method)->exchange_rate;

            // Set cart
            $amount                    = $this->calculateExchangeAmount($gateway_currency_exchange);
            
            // Save
            $webhook                 = new ProjectBidWebhook();
            $webhook->subscription_id           = $this->subscription->id;
            $webhook->bid_id           = $this->subscription->bid->id;
            $webhook->user_id           = $seller_id;
    ...
    public function wallet()
    {
        try {
            ...
            $bid_id = $this->subscription->bid->id;

            // Set amount
            $amount                  = $this->calculateExchangeAmount();
            ...
            $this->subscription->payment_method = 'wallet';
            $this->subscription->payment_id     = $deposit->transaction_id;
            $this->subscription->status         = 'paid';
            $this->subscription->save();

            // Let's take money from buyer's wallet
            auth()->user()->update([
                'balance_purchases' => convertToNumber(auth()->user()->balance_purchases) + convertToNumber($amount),
                'balance_available' => $available_balance - $amount
            ]);
