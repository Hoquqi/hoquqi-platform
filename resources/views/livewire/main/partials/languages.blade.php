<div class="relative inline-block">
    <div x-data="{ open: false }">
        ...
        {{-- Desktop Button --}}
        <button x-on:click="open = !open" type="button" class="hidden md:flex items-center text-gray-500 hover:text-primary-600 transition-colors duration-300 py-2 px-2 focus:outline-none focus:ring-0 dark:text-gray-100 dark:hover:text-white">
            <img src="{{ placeholder_img() }}" data-src="{{ countryFlag($default_country_code) }}" alt="{{ $default_language_name }}" class="lazy w-5 h-5">
        </button>

        {{-- Mobile Button --}}
        <button x-on:click="open = !open" type="button" class="flex md:hidden items-center text-gray-500 hover:text-primary-600 transition-colors duration-300 py-2 px-2 focus:outline-none focus:ring-0 dark:text-gray-100 dark:hover:text-white">
            <img src="{{ placeholder_img() }}" data-src="{{ countryFlag($default_country_code) }}" alt="{{ $default_language_name }}" class="lazy w-5 h-5">
        </button>
