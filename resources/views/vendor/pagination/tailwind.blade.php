                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $paginator->url($page) . (Str::contains($paginator->url($page), '&') ? '?' : '') . (http_build_query(request()->except('page')) ? '&'.http_build_query(request()->except('page')):'') }}" class="block w-8 h-8 leading-8 text-center border border-gray-100 dark:border-zinc-600 dark:hover:border-zinc-500 dark:text-gray-300 rounded-full text-xs font-medium" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
